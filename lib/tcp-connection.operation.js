...
class TcpConnectionOperation {
  ...
  _processSocket(info) {
    ...
    // TODO: ah-net needst to process the `server` of the activity so we
    // get goodies like the connection key
    /*
    if (resource != null
      && resource._connectionKey != null
      && resource._connectionKey.val != null) {
      socket.connectionKey = resource._connectionKey.val
    }
    */
  ...
  summary({ separateFunctions = true, mergeFunctions = true } = {}) {
    const timeAlive = prettyNs(this._destroyed.ns - this._created.ns)
    const info = {
        lifeCycle : {
