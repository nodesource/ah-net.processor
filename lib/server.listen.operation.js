...
class ServerListenOperation {
   ...
   * @function
   * @param {Object} info information about the open step, pre-processed by the `ServerListenProcessor`.
   */
  _processListen(info) {
    this._created = safeFirstStamp(info.activity.init)
    this._destroyed = safeFirstStamp(info.activity.destroy)
    this._timeAlive = this._destroyed.ns === 0
      ? prettyNs(0)
  ...
  summary({ separateFunctions = true, mergeFunctions = true } = {}) {
    // TODO: determine if we see a destroy when the server properly closed down
    const info = {
        lifeCycle: {
