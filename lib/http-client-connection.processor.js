...
class HttpClientConnectionProcessor extends ClientConnectionProcessorBase {
  ...
  _httpParserIdRelatedToSocket(socketId) {
    for (const parserId of this._httpParserIds) {
      ...
      const hasAsyncId = a.resource.socket._asyncId != null

      // newer Node.js versions don't attach _asyncId anymore :(
      if (!hasAsyncId) {
        const sock1 = this._activities.get(socketId).resource.owner
        const sock2 = a.resource.socket
        // XXX: may result in false positives, but at this point only option we have
        if (sock1._bytesDispatched === sock2._bytesDispatched) return parserId
      } else {
