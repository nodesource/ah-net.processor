...
class TcpClientConnectionProcessor {
   ...
   * @param {boolean} [separateFunctions=true] if `true` the functions will be pulled off each resource
   * and appended to the operation object
   */
...
const processor = new TcpClientConnectionProcessor({ activities })
const { groups } = processor.process()
inspect({ groups })
