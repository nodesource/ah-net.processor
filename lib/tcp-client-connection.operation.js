...
const processTls = require('./common/process-tls')
...
class TcpClientConnectionOperation {
   ...
   * @constructor
   */
  constructor({ group, includeActivities = false, isTls = false }) {
  ...
  _processSocket(info) {
    ...
    if (this._isTls) {
      if (initStack != null && initStack.length > 3) {
        this._createdAt = initStack[3]
      }
    } else {
      if (initStack != null && initStack.length > 2) {
        this._createdAt = initStack[2]
      }
    }
   ...
   *
   * @name tcpClientConnectionOperation._processTls
   * @function
   * @param {Object} info information about the tls wrap
   */
  _processTls(info) {
    this._tls = processTls(info)
  }
  ...
  summary({ separateFunctions = true, mergeFunctions = true } = {}) {
    ...
    if (this._httpParser != null) info.httpParser = this._httpParser
    if (this._isTls) info.tls = this._tls
